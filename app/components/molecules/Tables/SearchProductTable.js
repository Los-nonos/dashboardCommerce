...
class SearchProductTable extends React.Component {
  ...
  handleOrderBy = props => {
    ...
    if (orderBy !== null) {
      ...
      this.dispatch(
        this.props.changeOrder(this.props.searchProductState, this.props.searchProductState.page, orderBy, order),
      );
    }
  };
                ...
                return (
                  <TableRow key={key}>
                    {prop.visibleData.map((prop, innerKey) => {
                      return (
                        <TableCell className={classes.tableCell} key={innerKey}>
                          {typeof prop === 'boolean' ? (
                            prop ? (
